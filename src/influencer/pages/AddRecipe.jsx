...
import {
  ...
  FileText, Video, Leaf, Check, Upload,
  Stethoscope, Clock, Users, Flame, AlertCircle,
} from "lucide-react";
...
const CATEGORIES = [
  ...
  "Continental","Japanese","Korean","Mexican","Other",
];

const HERBAL_CATEGORIES = [
  "Hair","Face & Skin","Body","Immunity",
  "Digestion","Sleep","Weight","General Wellness",
];

const UNITS = ["g","kg","ml","l","tsp","tbsp","cup","oz","lb","pcs","cloves","pinch","to taste"];

const DOCTORS = [
  { id:"d1", name:"Dr. Priya Sharma",     spec:"Ayurvedic Physician",   avatar:"PS" },
  { id:"d2", name:"Dr. Arjun Mehta",      spec:"Naturopath",            avatar:"AM" },
  { id:"d3", name:"Dr. Sunita Rao",       spec:"Herbal Medicine Expert",avatar:"SR" },
...
const STEP_LABELS = ["Type","Basics","Ingredients","Instructions","Media","Review"];
...
const NavButtons = ({ onBack, onNext, nextLabel = "Next", nextType = "button", disabled = false }) => (
...
const AddRecipe = () => {
  ...
  const {
    ...
    defaultValues: {
      title:"", level:"", category:"", productLink:"", description:"",
      prepTime:"", cookTime:"",
      calories:"", protein:"", carbs:"", fat:"",
      steps:"",
      herbalCategory:"", benefit:"", herbs:"",
    ...
    return (
      <>
        ...
        <div className="max-w-xl mx-auto px-4 py-16 text-center"
          ...
          <h2 className="font-serif text-[26px] font-semibold text-stone-900 mb-2">Recipe Submitted!</h2>
...
`}</style>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8  min-h-screen">
        ...
        {/* ── Step bar ── */}
        <div style={{ animation:"fadeUp .4s .04s ease both", opacity:0, animationFillMode:"both" }}>
        ...
        {step === 2 && (
          <Card key={2}>
            ...
            <div className="flex flex-col gap-4">
              <Field label="Recipe Title" required error={errors.title}>
                <input
                  ...
                  placeholder={isHerbal ? "e.g. Amla & Bhringraj Hair Oil" : "e.g. Butter Chicken"}
                />
              </Field>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Difficulty Level" required error={errors.level}>
                  <div className="relative">
                    ...
                    <ChevronRight size={13} className="absolute right-3 top-1/2 -translate-y-1/2 rotate-90 text-stone-400 pointer-events-none" />
                  </div>
                </Field>

                <Field label={isHerbal ? "Remedy Category" : "Cuisine Category"} required error={errors.category}>
                  <div className="relative">
                    <select
              ...
              {isHerbal && (
                <>
                  <Field label="Primary Benefit" required error={errors.benefit}>
                    <input
                  ...
                  <Field label="Key Herbs Used" required error={errors.herbs} hint="Comma-separated list">
                    <input
                      {...register("herbs", { required:"List the herbs used" })}
              ...
              <Field label="Product / Reference Link" hint="Optional affiliate or source link">
                <input
            ...
            <NavButtons
              onBack={goBack}
              onNext={() => goNext(["title","level","category","description", ...(isHerbal ? ["benefit","herbs"] : [])])}
            />
          </Card>
        )}
        ...
        {step === 3 && (
          <Card key={3}>
            ...
            <div className="grid grid-cols-[1fr_80px_80px_auto] gap-2 mb-1.5 px-1">
              ...
              <p className="text-[10px] font-medium text-stone-400 uppercase tracking-wider">Amount</p>
              <p className="text-[10px] font-medium text-stone-400 uppercase tracking-wider">Unit</p>
              <span className="w-9" />
            </div>
        ...
        {step === 4 && (
          <Card key={4}>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                <p className="text-[12px] font-medium text-stone-600 uppercase tracking-wider mb-3">
                  {isHerbal ? "Dosage / Usage Info (Optional)" : "Nutritional Info (Optional)"}
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {(isHerbal
                    ? [
                        { name:"calories", placeholder:"Calories",  icon:<Flame size={12}/> },
                        { name:"protein",  placeholder:"Dosage",    icon:<Users size={12}/> },
                        { name:"carbs",    placeholder:"Frequency", icon:<Clock size={12}/> },
                        { name:"fat",      placeholder:"Duration",  icon:<Leaf  size={12}/> },
                      ]
                    : [
                        { name:"calories", placeholder:"Calories",     icon:<Flame size={12}/> },
                        { name:"protein",  placeholder:"Protein (g)",  icon:<Users size={12}/> },
                        { name:"carbs",    placeholder:"Carbs (g)",    icon:<Clock size={12}/> },
                        { name:"fat",      placeholder:"Fat (g)",      icon:<Flame size={12}/> },
                      ]
                  ).map(f => (
            ...
            <div className="flex flex-col gap-5">
                  ...
                  {videoPreview ? (
                    ...
                    >
                      ...
                      <div className="text-center">
                        <p className="text-[13px] font-medium text-stone-700">Click to upload video</p>
                        <p className="text-[11px] text-stone-400 mt-0.5">MP4, MOV, AVI up to 500 MB</p>
                      </div>
                    </button>
                  )}
                  <input
              ...
              {recipeType === "text" && (
                <div>
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p className="text-[13px] font-medium text-stone-700">Click to upload image</p>
                      <p className="text-[11px] text-stone-400 mt-0.5">JPG, PNG, WebP up to 10 MB</p>
                    </div>
                  </button>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { key:"public",  label:"Public",  desc:"Everyone can see this recipe." },
                    { key:"private", label:"Private", desc:"Only you can see this recipe." },
                  ].map(v => (
                    <button
                      key={v.key}
                      type="button"
        ...
        {step === 6 && (
          <Card key={6}>
            ...
            <p className="text-[13px] text-stone-400 mb-4">
              All recipes must be verified by a qualified practitioner before publishing.
            </p>
            ...
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3.5 mb-6">
              ...
              <div>
                <p className="text-[12px] font-semibold text-amber-800 mb-0.5">Verification Required</p>
            ...
            {/* Doctor selection */}
            <p className="text-[12px] font-medium text-stone-600 uppercase tracking-wider mb-3">
              Select a Verifying Doctor <span className="text-red-400">*</span>
            </p>
            ...
            <div className="bg-stone-50 border border-stone-100 rounded-xl p-4 mb-1">
              ...
              {[
                ...
                { label:"Type",     value: recipeType === "video" ? "Video Recipe" : "Text Recipe" },
                { label:"Category", value: isHerbal ? "Herbal / Ayurvedic" : "Regular"             },
                { label:"Level",    value: watch("level")       },
                { label:"Cuisine",  value: watch("category")    },
                { label:"Visibility",value: visibility          },
                ...(isHerbal ? [{ label:"Benefit", value: watch("benefit") }] : []),
              ].filter(r => r.value).map(row => (
                <div key={row.label} className="flex justify-between py-1.5 border-b border-stone-100 last:border-0">
                  <span className="text-[12px] text-stone-400">{row.label}</span>
